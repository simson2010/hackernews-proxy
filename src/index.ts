...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (storiesMatch) {
    ...
    if (isNaN(limit) || limit < 1) return sendJson(res, 400, { error: 'Limit must be between 1 and 100' });

    const ids = await (await fetch(`${HN_API_BASE}/${storyType}stories.json`)).json() as number[];
    const stories = await Promise.all(
      ids.slice(0, limit).map((id) => fetch(`${HN_API_BASE}/item/${id}.json`).then((r) => r.json()))
    );
